...
enum MemoryFlags : uint32_t {
  ...
  //! A combination of `kAccessRead | kAccessExecute`.
  kAccessReadExecute = kAccessRead | kAccessExecute,
...
enum HardenedRuntimeFlags : uint32_t {
  //! Hardened runtime is enable - it's not possible to have "Write & Execute"
  //! memory protection. The runtime enforces W^X.
  //!
  ...
  //! also a possibility on X86_64 architecture.
